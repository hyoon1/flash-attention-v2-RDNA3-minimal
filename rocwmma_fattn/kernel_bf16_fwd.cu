        ...
        const float32_t scale, const bool permute_NH)
{
    ...
    extern __shared__ ComputeType_Out sram[];
    ComputeType_Out *__restrict__ Si = &sram[0];       // Br * Bc
    ComputeType_Out *__restrict__ Oi = &sram[Br * Bc]; // Br * d
...
#pragma unroll 2
            ...
            {
                float_v16 val = FLOATV16(Si[(tx * Bc) + i]); 
#pragma unroll
                for (int j = 0; j < 16; j++)
                    val32 = max(val32, val[j]); // V_PK_MAX_F16
            }
            row_max_new = val32;
...
#pragma unroll // convert back to fp16
               ...
               // write back
                HALF16((reinterpret_cast<ComputeType*>(Si))[(tx * Bc) + i]) = val;
            }
