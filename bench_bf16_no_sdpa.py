...
def fttn_ck(q, k, v=None):
    ...
    #ret = flash_attn_func(q2,k2,v2,0,sc,causal,(-1,-1))
    #O = (ret[0])[:, :, :, :d_qkv]
    O = ret
    #O = O.transpose(1, 2)
...
for i in range(0,5,1):
    ...
    maxdiff = (r1 - r3).abs().max().item()
    print("max diff triton-rocwmma: ", maxdiff)
    maxdiff = (r1 - r2).abs().max().item()
    print("max diff triton-rocwmma_fixed: ", maxdiff)
    #maxdiff = (r0 - r1).abs().max().item()
    #print("max diff sdp-triton: ", maxdiff)
    maxdiff = (r1 - r4).abs().max().item()
    print("max diff triton-ck: ", maxdiff)
...
plt.plot(n_list, flops_ck_list, label="flash-attn-ck")
plt.xlabel("Seqlen")
plt.ylabel('TFlops')
plt.legend()
plt.xticks(n_list)
...
plt.plot(n_list, maxmem_ck_list, label="flash-attn-ck")
plt.xlabel("Seqlen")
plt.ylabel('VRAM(MB)')
plt.legend()
plt.xticks(n_list)
...
fig.savefig('fwd_scan_N_all_2.png')
